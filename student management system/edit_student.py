...
def fun2():
    ...
    try:
        ...
        cur=conn.cursor()
        s4="update `friends` set `name`='"+s1+"',`email`='"+s2+"',`phone`='"+s3+"' where `id`='"+n2+"'"
        res1=cur.execute(s4)
...
def display(n,n3):
    ...
    B4 = tk.Button(frame2, text="Update" ,fg="white",bg="#262523" ,width=29, command=fun2 ,height=1,font=('times', 16, ' bold '))
    B4.place(relx=0, rely=0.66, relwidth=1, relheight=0.10)
